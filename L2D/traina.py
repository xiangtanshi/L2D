...
class Args:
    def __init__(self, seed = 0, lr = 1e-3, bs = 128, momentum = 0.9, weight_decay = 5e-4, ep = 20, clss = 10, 
                model_path = './models/resnet/nico_animal.pkl', optimizer = 'adam', 
                data = [a_ani_train, a_ani_valid, a_ani_test], load_flag = True):
...
class Trainer:
    ...
    def __init__(self, args):
        ...
        if args.load_flag:
            ...
            self.val_loader = DataLoader(dataset=args.data[1], batch_size=args.batchsize, shuffle=False, num_workers=4)
            self.test_loader = DataLoader(dataset=args.data[2], batch_size=args.batchsize, shuffle=False, num_workers=4)
        ...
        for itr in range(self.args.epoch):
            ...
            if itr>14:
                if isinstance(self.model, nn.DataParallel) or isinstance(self.model, nn.parallel.DistributedDataParallel):
                    # multipul gpu
                    torch.save(self.model.module.state_dict(),'./models/inference/model_4_{}.pkl'.format(int(itr)))
                else:
                    torch.save(self.model.state_dict(),'./models/inference/model_4_{}.pkl'.format(int(itr)))
...
if __name__ == '__main__':
    ...
    args = Args(seed=0,bs=128, clss=10, optimizer='adam', data=[a_ani_train, a_ani_valid, a_ani_test])
    # train classifiers
    model_path='./models/resnet/nico_animal.pkl',
    args.lr = 1e-3
    trainer = Trainer(args)
    trainer.training(method='normal')
