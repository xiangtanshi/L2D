...
class ResNet(nn.Module):
    def __init__(self, block=BasicBlock, layers=[2, 2, 2, 2], num_classes=10):
        # change params: block and layers to swift from res18 to other models 
        self.inplanes = 64
        ...
        self.register_buffer('pre_features', torch.zeros(128, 512))
    ...
    def forward(self, x, gt=0, flag=False, epoch=0):
        ...
        if flag:
            ...
            x_new_view_after = self.fc_(x_new_view_after)
...
def main():
    #resnet18 = models.resnet18(pretrained=True)    # downloading pretrained resnet to local env

    resnet18 = models.resnet18(pretrained=False)
    resnet18.load_state_dict(torch.load('./models/resnet/resnet18-5c106cde.pth'))

    model_a = ResNet(num_classes=10)      # animal
    model_v = ResNet(num_classes=8)       # vehicle
    pretrained_dict = resnet18.state_dict()
    model_dict_a = model_a.state_dict()
    model_dict_v = model_v.state_dict()

    pretrained_dict_a = {k:v for k,v in pretrained_dict.items() if k in model_dict_a}
    pretrained_dict_v = {k:v for k,v in pretrained_dict.items() if k in model_dict_v}
    model_dict_a.update(pretrained_dict_a)
    model_dict_v.update(pretrained_dict_v)
    model_a.load_state_dict(model_dict_a)
    model_v.load_state_dict(model_dict_v)

    torch.save(model_a.state_dict(),'./models/resnet/nico_animal.pkl')
    torch.save(model_v.state_dict(),'./models/resnet/nico_vehicle.pkl')
